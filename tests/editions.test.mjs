...
test('sélecteur annuel, blocage pendant une saisie et ajout sans doublon',async()=>{
 ...
 assert.equal(document.querySelector('summary').textContent,'2026⌄');assert.equal(document.querySelector('summary').textContent.includes('LCS'),false);document.querySelector('.edition-option').click();assert.equal(selected.length,0);leave=true;document.querySelector('.edition-option').click();assert.deepEqual(selected,[2027]);
