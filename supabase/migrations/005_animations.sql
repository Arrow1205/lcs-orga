-- Après 004_bilan.sql ; une seule fois. Les animations sont propres à chaque année.
begin;
lock table public.crm_v2_status in access exclusive mode;
create table public.crm_animations (
 ...
 day text generated always as (payload->>'day') stored,
 start_time text generated always as (payload->>'start') stored,
 end_time text generated always as (payload->>'end') stored,
 owner text generated always as (payload->>'owner') stored,
 "partnerId" text generated always as (payload->>'partnerId') stored
);
create index on public.crm_animations(workspace_id,edition_year,day,start_time);
alter table public.crm_animations enable row level security;
revoke all on public.crm_animations from anon,authenticated;
grant select on public.crm_animations to authenticated;
create policy member_read on public.crm_animations for select to authenticated
...
begin
 ...
 foreach entity in array array['exhibitors','contacts','partners','tasks','posts','expenses','ideas','assets','invoices','zones','ledger_entries','ledger_categories','animations'] loop
...
begin
 ...
 for change in select value from jsonb_array_elements(p_changes) loop
  ...
  if entity is null or not(entity=any(array['exhibitors','contacts','partners','tasks','posts','expenses','ideas','assets','invoices','zones','ledger_entries','ledger_categories','animations','settings','forecast','metadata'])) or key is null or key='' then raise exception 'Table ou identifiant invalide' using errcode='22023'; end if;
  token:=entity||':'||key;
  ...
  if entity=any(array['exhibitors','contacts','partners','tasks','posts','expenses','ideas','assets','invoices','zones','ledger_entries','ledger_categories','animations']) and after_value is not null and after_value->>'id' is distinct from key then raise exception 'Identifiant incohérent'; end if;
